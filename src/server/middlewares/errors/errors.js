  ...
  if (error.error === "Bad Request") {
    ...
    if (
      ...
      res
        ...
        .json({ message: "La contraseña debe ser entre 8 y 30 carácteres" });
    }
    if (body[0].type === "string.alphanum") {
      if (body[0].path[0] === "password") {
        res
          .status(error.statusCode)
          .json({ message: "La contraseña debe ser alfanumérica" });
      }
      if (body[0].path[0] === "username") {
        res
          .status(error.statusCode)
          .json({ message: "El username debe ser alfanumérico" });
      }
      if (body[0].path[0] === "name") {
        res
          .status(error.statusCode)
          .json({ message: "El nombre debe ser alfanumérico" });
      }
    }
    if (body[0].type === "string.email" && body[0].path[0] === "email") {
      res.status(error.statusCode).json({ message: "El email no es válido" });
    }
  }
